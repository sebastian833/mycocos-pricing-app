...
import { FileSpreadsheet, CheckCircle, AlertCircle, BarChart2, Sliders, MessageCircle } from 'lucide-react'
import { parseShopyLibreFile } from '@/lib/parser-client'

export default function Home() {
  const { data, setData, setFileName, marcaActual, loading: cargandoMarca } = useData()
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      setData(parsed)
      setFileName(`${marcaActual.nombre} — ${file.name}`)
    } catch (e: unknown) {
  ...
  if (cargandoMarca) return null

  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-10">
        <div className="inline-flex items-center justify-center w-14 h-14 bg-blue-100 rounded-2xl mb-4 text-3xl">
          {marcaActual.emoji}
        </div>
        <h1 className="text-2xl font-semibold text-gray-900 mb-2">{marcaActual.nombre} — Pricing Dashboard</h1>
        <p className="text-gray-500 text-sm">
          {data
            ? 'Estos son los datos cargados para esta marca'
            : 'Aún no hay datos cargados para esta marca — sube el reporte de ventas para comenzar'}
        </p>
      </div>
      ...
      {!data ? (
        ...
        >
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="font-medium text-gray-700">Arrastra el reporte de {marcaActual.nombre} aquí</p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <p className="font-medium text-gray-900">Datos de {marcaActual.nombre} listos</p>
          ...
          <button onClick={() => { setData(null); setFileName('') }} className="w-full text-sm text-gray-400 hover:text-gray-600 transition-colors">
            Cargar otro archivo para {marcaActual.nombre}
          </button>
        </div>
      )}
